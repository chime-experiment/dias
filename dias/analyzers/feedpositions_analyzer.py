...
# Choose 10 good frequencies. I chose the same ones that we used when
# writing full N2 data for 10 frequencies.
...
class FeedpositionsAnalyzer(CHIMEAnalyzer):
        ...
        for night_source in night_transits:
            ...
            for i in range(0, NCYL * NPOL, NPOL):
                ew_offsets[:, i *
                           NCYLPOL:(i +
                                    1) *
                           NCYLPOL] *= np.median(ew_positions[:, i *
                                                              NCYLPOL:(i +
                                                                       1) *
                                                              NCYLPOL], axis=1)[:, np.newaxis]
                ew_offsets[:, (i + 1) * NCYLPOL:(i + 2) * NCYLPOL] *= np.median(
                    ew_positions[:, (i + 1) * NCYLPOL:(i + 2) * NCYLPOL], axis=1)[:, np.newaxis]
    ...
    def east_west_positions(self, src):
        ...
        for i in range(len(freq_sel)):
            ...
            if np.all(ratio < 2):
                count += 1
                self.logger.warn(
                    "Eigenvalue ratio on source versus off source smaller than 2. Suspecting RFI contamination for this frequency " +
                    str(
                        freq_sel[i]))
        ...
        self.logger.info(
            'Exporting number of good frequencies (non RFI contaminated) in this data to Prometheus')
        ...
        # Reference eigenvector to the first good feed for the NS(P1) and
        # EW(P2) polarisation
        for i in range(0, NCYL * NPOL, NPOL):
            evec[:,
        ...
        for f in range(len(freq_sel)):
            for i in range(NINPUT):
                ew_positions[f, i], resolution[f, i] = self.get_ew_pos_fft(
                    time, evec[f, i, :], freq[f], np.radians(dec), pad_fac=self.pad_fac_EW)
    ...
    def orthogonalize(self, data, fsel, time_index):
        ...
        Vtx = Lh[:, np.newaxis] * \
            np.dot(U.T.conj(), Ax[:, np.newaxis] * U) * Lh[np.newaxis, :]
        m, vv = la.eigh(Vtx)
        vx = Ax[:, np.newaxis] * np.dot(U, vv / Lh[:, np.newaxis])
        vx /= np.dot(vx.T.conj(), vx).diagonal()**0.5

        Vty = Lh[:, np.newaxis] * \
            np.dot(U.T.conj(), Ay[:, np.newaxis] * U) * Lh[np.newaxis, :]
        m, vv = la.eigh(Vty)
